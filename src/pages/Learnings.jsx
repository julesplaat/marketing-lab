...
import {
  PageHeader, Card, Badge, Select, Tabs, EmptyState, ConfirmDialog,
...
export default function Learnings() {
  ...
  const [filters, setFilters] = useState({
    funnel: '', channel: '', testType: '', buyerRole: '', problemTag: '', applyTo: '', confidence: '',
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      items = items.filter((l) =>
        [l.title, l.body, l.funnel, l.channel, ...(l.buyerRoles || []), ...(l.problemTags || []), ...(l.applyTo || [])].join(' ').toLowerCase().includes(q)
      );
    }
    ...
    switch (quickView) {
      case 'hooks':
        items = items.filter((l) => l.testType === 'Hook');
        break;
      case 'ctas':
        items = items.filter((l) => l.testType === 'CTA');
        break;
      case 'formats':
        items = items.filter((l) => l.testType === 'Format');
        break;
      case 'roles':
        items = items.filter((l) => (l.buyerRoles || []).length > 0);
        break;
      case 'objections':
        items = items.filter((l) => {
          const exp = state.experiments.find((e) => e.id === l.experimentId);
          return exp?.evaluation?.objectionsSeen;
        });
        break;
      case 'homepage':
        items = items.filter((l) => (l.applyTo || []).includes('Homepage'));
        break;
      case 'linkedin':
        items = items.filter((l) => (l.applyTo || []).includes('LinkedIn'));
        break;
      case 'sales':
        items = items.filter((l) => (l.applyTo || []).includes('Salesmateriaal'));
        break;
    }
  ...
  return (
    <div className="page">
      <PageHeader
        title="Learnings"
        subtitle={`${state.learnings.length} learnings opgeslagen`}
        actions={
          <button className="btn btn-primary" onClick={() => setEditModal({
            title: '', body: '', experimentId: '', funnel: '', channel: '',
            buyerRoles: [], problemTags: [], testType: '', applyTo: [], confidence: 'medium',
          })}>
            <Plus size={16} /> Nieuwe learning
          </button>
        }
      />
      ...
      <div className="quick-view-bar">
        {QUICK_VIEWS.map((qv) => (
          <button
            key={qv.value}
            className={`quick-view-btn ${quickView === qv.value ? 'active' : ''}`}
            onClick={() => setQuickView(qv.value)}
          >
            {qv.label}
          </button>
        ))}
        ...
        <div className="filters-panel">
          <div className="filters-grid">
            <div className="filter-item">
              <label>Funnel</label>
              <Select value={filters.funnel} onChange={(v) => setFilters({ ...filters, funnel: v })} options={FUNNEL_OPTIONS} placeholder="Alle" />
            </div>
            <div className="filter-item">
              <label>Kanaal</label>
            ...
            <div className="filter-item">
              ...
              <Select value={filters.confidence} onChange={(v) => setFilters({ ...filters, confidence: v })} options={CONFIDENCE_OPTIONS} placeholder="Alle" />
            </div>
            <div className="filter-item">
              <label>Toepassing</label>
              <Select value={filters.applyTo} onChange={(v) => setFilters({ ...filters, applyTo: v })} options={APPLY_TO_OPTIONS} placeholder="Alle" />
            </div>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Evalueer experimenten om learnings vast te leggen."
        />
      ) : (
        <div className="learnings-list">
          {filtered.map((l) => {
            const exp = state.experiments.find((e) => e.id === l.experimentId);
            return (
              <Card key={l.id} className="learning-card">
                <div className="learning-header">
                  <h3 className="learning-title">{l.title}</h3>
                  <div className="learning-actions">
                    <button className="btn-icon" onClick={() => setEditModal(l)} title="Bewerken"><Edit size={14} /></button>
                    <button className="btn-icon btn-danger-text" onClick={() => setDeleteConfirm(l.id)} title="Verwijderen"><Trash2 size={14} /></button>
                  </div>
                </div>
                ...
                <div className="learning-meta">
                  ...
                  {l.confidence && (
                    <Badge small color={l.confidence === 'high' ? 'green' : l.confidence === 'medium' ? 'yellow' : 'gray'}>
                      {l.confidence === 'high' ? 'Hoog' : l.confidence === 'medium' ? 'Middel' : 'Laag'} vertrouwen
                    </Badge>
                  )}
                </div>
                <div className="learning-footer">
                  {(l.applyTo || []).length > 0 && (
                    <span className="learning-apply">Toepassen op: {l.applyTo.join(', ')}</span>
                  )}
                  <span className="learning-date">{formatDate(l.createdAt)}</span>
                  {exp && (
                    <button className="btn btn-ghost btn-sm" onClick={() => navigate(`/experiments/${exp.id}`)}>
                      <ExternalLink size={12} /> {exp.title}
                    </button>
                  )}
                </div>
              </Card>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { deleteLearning(deleteConfirm); setDeleteConfirm(null); }}
        title="Learning verwijderen"
        message="Weet je zeker dat je deze learning wilt verwijderen?"
  ...
  return (
    <Modal open onClose={onClose} title={form.id ? 'Learning bewerken' : 'Nieuwe learning'} wide>
      <div className="form-grid form-grid-single">
        ...
        <FormField label="Learning">
          <Textarea value={form.body} onChange={(v) => update('body', v)} rows={4} />
        </FormField>
      </div>
      <div className="form-grid">
        <FormField label="Bron-experiment">
          <Select
            value={form.experimentId}
            onChange={(v) => update('experimentId', v)}
      ...
      <div className="form-grid form-grid-single">
        <FormField label="Buyer roles">
          <MultiSelect value={form.buyerRoles || []} onChange={(v) => update('buyerRoles', v)} options={BUYER_ROLE_OPTIONS} />
        </FormField>
        <FormField label="Toepassen op">
          <MultiSelect value={form.applyTo || []} onChange={(v) => update('applyTo', v)} options={APPLY_TO_OPTIONS} />
        </FormField>
      </div>
